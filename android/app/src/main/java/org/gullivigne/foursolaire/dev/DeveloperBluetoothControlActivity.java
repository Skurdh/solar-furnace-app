...
import org.gullivigne.foursolaire.dev.controler.Controler;
...
//        ArrayList<ReceiverMonitor> receivers = SaveManager.getInstance(this).getReceivers();
//        if (receivers != null) {
//            for (ReceiverMonitor receiver : receivers) {
...
//        }
        ...
        for (int i = 0; i < 9; i++) {
            Controler newControl = new Controler("Btn " + String.valueOf(i), "", i);
    ...
    public void writeMessage(String msg) {
        if (!msg.isEmpty()) {
            TerminalMessage newTerminalMsg = new TerminalMessage(getCurrentTime(), msg, TerminalMessage.ANDROID, TerminalMessage.TYPE_REQUEST);
            adapterTerminal.addMessage(newTerminalMsg);
            if (setting_autoscroll) {
                terminalRecyclerView.scrollToPosition(adapterTerminal.getItemCount() - 1);
            }
